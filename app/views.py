...
def logout():
    logout_user()
    return redirect(url_for('login'))  # Reindirizza l'utente alla pagina di login dopo il logout
...
def login():
    if current_user.is_authenticated:  # Controlla se l'utente è già loggato
        return redirect(url_for('dashboard'))  # Redirigi alla dashboard se già loggato
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            login_user(user)
            return redirect(url_for('dashboard'))  # Redirigi all'area protetta dopo il login
        else:
